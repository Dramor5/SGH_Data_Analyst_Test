...
Interestingly, the number of diagnoses outnumber the number of people with diabetes. Could this be a misdiagnosis by the physician or was a lower gh threshold used to diagnose them? The use of the dx feature in the model may add some false-positives according to our criteria but should still be useful as their proportions are very similar. Of course, this is assuming the dx and diabetes statuses have a significant correlation. Following the same logic, tx should be a better predictor - the difference in gh and tx proportions are minimal.

At this point I'm unsure if the other features are as valuable, we would have to look for correlations with diabetes to determine this.

We move on to correlation visualizations to investigate if any features are correlated with diabetes. 
...
There were positive relationships between anthropometric variables such as ht and leg, wt and bmi but these are not very helpful. 
...
```{r}
# Open a new list for result collection
t_test_df <- list()
...
I will do the same with categorical variables, using the chi-squared test of independence. 
...
```
As expected, there is a high correlation between tx, dx and diabetes. Since all the variables in this dataset are all somehow contributing to diabetes, there were significant differences between means of the continuous variables in diabetics and non-diabetics. All categorical variables were also correlated to diabetes. Feature selection is still important to reduce the dimensionality in data to create a more effective model. 

tx, dx, age (after removing below 20), bmi (since it tell us the most information about a person's body shape, and is also correlated with several other features.)
