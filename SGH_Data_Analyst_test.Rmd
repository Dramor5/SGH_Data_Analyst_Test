...
# Make a ROCR prediction object to evaluate the model.
ROCR_pred <- prediction(predictions = log_predictions,
                        labels = test_df$diabetes)

# Obtain accuracy, precision, recall and AUC.
accuracy <- performance(ROCR_pred, measure = "acc")@y.values[[1]]
precision <- performance(ROCR_pred, measure = "ppv")@y.values[[1]]
recall <- performance(ROCR_pred, measure = "sens")@y.values[[1]]
auc <- performance(ROCR_pred, measure = "sens")@y.values[[1]]
